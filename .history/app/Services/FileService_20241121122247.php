...
/*
  public function getAllFiles($perPage = 10)
    {
        $fetchedFile File::get()->paginate($perPage);
...
public function getAllFiles($perPage = 10)
{
    ...
    // احصل على الملفات باستخدام الترقيم
    $paginatedFiles = $filesQuery->paginate($perPage);

    $authorizedFiles = []; // قائمة الملفات المصرح بها

    foreach ($paginatedFiles as $file) {
        try {
            // قم بجلب الملف باستخدام التخزين المؤقت والصلاحيات
            $fileInstance = File::fetchByIdWithCacheAndAuth($file->id);

            // تحقق من الصلاحية باستخدام Gate
            if (Gate::allows('viewFile', $fileInstance)) {
                $authorizedFiles[] = $fileInstance;
            }
        } catch (\Exception $e) {
            // تجاوز الأخطاء (مثل الكائنات غير المصرح بها)
            continue;
        }
    }

    // تجهيز استجابة البيانات
    $response = [
        'success' => true,
        'data' => [
            'current_page' => $paginatedFiles->currentPage(),
            'data' => $authorizedFiles,
            'first_page_url' => $paginatedFiles->url(1),
            'from' => $paginatedFiles->firstItem(),
            'last_page' => $paginatedFiles->lastPage(),
            'last_page_url' => $paginatedFiles->url($paginatedFiles->lastPage()),
            'links' => $paginatedFiles->linkCollection(),
            'next_page_url' => $paginatedFiles->nextPageUrl(),
            'path' => $paginatedFiles->path(),
        ...
        'message' => 'api.file.getAllFiles.success',
    ];
