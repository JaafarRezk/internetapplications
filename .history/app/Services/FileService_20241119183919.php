...
public function viewAllFiles($perPage = 10)
{
    ...
    foreach ($paginatedFiles as $file) {
        try {
            // تحقق من الصلاحيات باستخدام Gate قبل إضافة الملف
            Gate::authorize('view', $file);  // التحقق من صلاحية المستخدم في سياسة 'view'

            // إذا كان المستخدم لديه الصلاحية، أضف الملف إلى المصفوفة
            $files[] = File::fetchByIdWithCacheAndAuth($file->id);
        } catch (\Exception $e) {
            // إذا لم يكن للمستخدم صلاحية عرض الملف، تجاهل الملف
            continue;
        }
    }
