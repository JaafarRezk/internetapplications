...
public function getAllFiles($perPage = 10)
{
    // استعلام الملفات بالتجزئة
    $paginatedFiles = File::paginate($perPage);

    $allowedFiles = [];

    foreach ($paginatedFiles as $file) {
        try {
            // التحقق من الصلاحيات باستخدام fetchByIdWithCacheAndAuth
            $allowedFiles[] = File::fetchByIdWithCacheAndAuth($file->id);
        } catch (\Exception $e) {
            // تجاهل الملفات التي لا يمتلك المستخدم صلاحية عرضها
            continue;
        }
    }

    // إعادة الملفات مع معلومات التجزئة
    return [
        'data' => $allowedFiles,
        'pagination' => [
            ...
            'total' => $paginatedFiles->total(), // العدد الإجمالي للملفات
        ],
    ];
}
