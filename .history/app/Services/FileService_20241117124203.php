...
{
    // استرجاع جميع معرفات الملفات
    $fileIds = File::pluck('id'); 

    $fileObjects = [];
    foreach ($fileIds as $id) {
        try {
            // استرجاع الملف باستخدام التابع fetchByIdWithCacheAndAuth
            $fileObjects[] = File::fetchByIdWithCacheAndAuth($id);
        } catch (\Exception $e) {
            // في حالة حدوث استثناء، الاستمرار بالملفات الأخرى
            continue;
        }
    }

    // إرجاع الملفات المسترجعة
    return $fileObjects;
}
