...
{
    // استرجاع جميع الملفات من قاعدة البيانات
    $files = File::all(); // جلب جميع الملفات في قاعدة البيانات

    // تأكد من أن المستخدم يملك صلاحية الوصول لكل ملف
    $fileObjects = [];
    foreach ($files as $file) {
        try {
            // استخدام التابع fetchByIdWithCacheAndAuth لجلب الملف مع الكاش والتحقق من الصلاحية
            $fileObjects[] = File::fetchByIdWithCacheAndAuth($file->id);
        } catch (\Exception $e) {
            // التعامل مع الأخطاء مثل عدم وجود صلاحية للوصول أو الملف غير موجود
            continue;
        }
    }
