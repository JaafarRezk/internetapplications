...
public function viewAllFiles()
{
    ...
    // تصفية الملفات بناءً على صلاحية العرض باستخدام fetchByIdWithCacheAndAuth
    $allowedFiles = $files->filter(function ($file) {
        try {
            // التحقق التلقائي باستخدام fetchByIdWithCacheAndAuth
            File::fetchByIdWithCacheAndAuth($file->id);
            return true; // إذا لم يتم رمي استثناء فإن الملف مسموح به
        } catch (\Exception $e) {
            // في حال تم رمي استثناء يعني أن الملف غير مسموح به
            return false;
        }
    });
