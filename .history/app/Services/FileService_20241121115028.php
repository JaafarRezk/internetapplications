...
public function getAllFiles($perPage = 10)
{
    // استعلام الملفات مع تقسيم الصفحات
    $filesQuery = File::query();
    $paginatedFiles = $filesQuery->paginate($perPage);
    ...
    foreach ($paginatedFiles as $file) {
        try {
            // جلب البيانات مع الكاش والصلاحيات
            $fetchedFile = File::fetchByIdWithCacheAndAuth($file->id);

            // التحقق من صلاحيات المستخدم
            if (Gate::allows('viewFile', $fetchedFile)) {
                $accessibleFiles[] = $fetchedFile;
            }
        } catch (\Exception $e) {
            // استمر في حالة وجود استثناء
            continue;
        }
    }

    // إرجاع البيانات المسموح بها
    return [
        'files' => $accessibleFiles,
        'pagination' => [
            'currentPage' => $paginatedFiles->currentPage(),
            'lastPage' => $paginatedFiles->lastPage(),
            'perPage' => $paginatedFiles->perPage(),
