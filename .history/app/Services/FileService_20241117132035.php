...
{
    // استرجاع جميع الملفات باستخدام pagination من قاعدة البيانات
    $filesQuery = File::query(); // استعلام عام لجلب الملفات

    // استخدام pagination لاسترجاع البيانات بشكل فعال
    $paginatedFiles = $filesQuery->paginate($perPage); 

    // الاستفادة من الكاش واسترجاع البيانات باستخدام `fetchByIdWithCacheAndAuth`
    $files = [];
    foreach ($paginatedFiles as $file) {
        try {
            // استخدام التابع fetchByIdWithCacheAndAuth لاسترجاع كل ملف مع الكاش والتحقق من الصلاحيات
            $files[] = File::fetchByIdWithCacheAndAuth($file->id);
        } catch (\Exception $e) {
            // التعامل مع الأخطاء في حال كان هناك خطأ في أي ملف
            continue; // استكمال العملية رغم وجود خطأ في أحد الملفات
        }
    }

    // إرجاع الملفات المحملة مع الـ pagination
    return $paginatedFiles; // `paginate` تعيد البيانات مع الـ pagination بشكل مباشر
}
