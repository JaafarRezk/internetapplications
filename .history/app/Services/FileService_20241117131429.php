...
public function getAllFilesWithCache($perPage = 10)
{
    $files = File::where('user_id', auth()->id())->paginate($perPage);

    // لتخزين الملفات في الكاش إذا كنت ترغب
    foreach ($files as $file) {
        // تخزين الكاش باستخدام الدالة من GenericModel
        Cache::put('model_cache:File:' . $file->id, $file, env('CACHE_EXPIRY', 600));
    }
